...
import { createNewMemorial } from '@/lib/memorial-utils';
import { getMemorialBySlug } from '@/lib/memorial-data-utils';
...
export const useMemorial = (slug?: string) => {
  ...
  const createMemorial = useMutation({
    mutationFn: async (data: MemorialFormData) => {
      ...
      const memorial = await createNewMemorial(data, locationInfo.is_brazil);
